...
const spell: Spell = {
    card: {
        ...
        effect: async (state, card, quantity, underworld, prediction) => {
            ...
            let promises = [];
            for (let unit of state.targetedUnits.filter(u => u.alive).filter(u => u.modifiers[burnCardId])) {
...
export async function contaminate(casterPlayer: IPlayer | undefined, unit: IUnit, quantity: number, radiusBoost: number, underworld: Underworld, prediction: boolean) {
    ...
    // we should only spread the initially targeted unit' burn stacks
    ...
    // multicasting allows Wildfire to chain off of nearby enemies
    let recursions = 0;
    ...
    for (let curse of curses) {
        ...
        for (let touchingUnit of nearbyUnits) {
            ...
            if (!prediction) {
                ...
                animationPromise = createVisualFlyingProjectile(
                    unit,
                    touchingUnit,
                    'poisonerProjectile',
                )
